(() => {
      ...
      var Delta13 = class _Delta {
        ...
        eachLine(predicate, newline = "\n") {
          ...
          while (iter.hasNext()) {
            ...
            const start3 = Op_1.default.length(thisOp) - iter.peekLength();
            const index = typeof thisOp.insert === "string" ? thisOp.insert.indexOf(newline, start3) - start3 : -1;
            if (index < 0) {
              line.push(iter.next());
            } else if (index > 0) {
    ...
    "../../vendor/assets/javascripts/activeadmin_quill_editor.js"(exports2, module2) {
      (function() {
        ...
        function initQuillEditor(container) {
          ...
          if (!content)
            return;
          const options = container.getAttribute("data-options") ? JSON.parse(container.getAttribute("data-options")) : {};
          if (!options.theme)
            options.theme = defaultTheme;
          if (!options.modules)
            options.modules = {};
          if (!options.modules.toolbar)
            options.modules.toolbar = defaultToolbar;
          const pluginOptions = container.getAttribute("data-plugins") ? JSON.parse(container.getAttribute("data-plugins")) : {};
          ...
          return {
            upload: function(file) {
              return new Promise(function(resolve, reject) {
                ...
                const headers = {};
                if (csrfToken2) {
                  headers["X-CSRF-Token"] = csrfToken2.getAttribute("content");
                }
                fetch(serverUrl, {
                  method: "POST",
                  body: formData,
  ...
  var _LeafBlot = class _LeafBlot2 extends ShadowBlot {
    ...
    position(index, _inclusive) {
      ...
      return index > 0 && (offset2 += 1), [this.parent.domNode, offset2];
    }
    /**
  ...
  var LinkedList = class {
    ...
    forEachAt(index, length, callback) {
      if (length <= 0)
        return;
      const [startNode, offset2] = this.find(index);
      let curIndex = index - offset2;
      ...
      for (; cur && curIndex < index + length; ) {
        ...
        ) : callback(cur, 0, Math.min(curLength, index + length - curIndex)), curIndex += curLength, cur = next2();
      }
    }
    map(callback) {
  ...
  var _ParentBlot = class _ParentBlot2 extends ShadowBlot {
    ...
    deleteAt(index, length) {
      if (index === 0 && length === this.length())
        return this.remove();
      this.children.forEachAt(index, length, (child, offset2, childLength) => {
        child.deleteAt(offset2, childLength);
      });
    }
    descendant(criteria, index = 0) {
      const [child, offset2] = this.children.find(index);
    ...
    path(index, inclusive = false) {
      const [child, offset2] = this.children.find(index, inclusive), position = [[this, index]];
    ...
    split(index, force = false) {
      ...
      return this.parent && this.parent.insertBefore(after, this.next || void 0), this.children.forEachAt(index, this.length(), (child, offset2, _length) => {
  ...
  var _InlineBlot = class _InlineBlot2 extends ParentBlot$1 {
    ...
    optimize(context) {
      ...
      if (Object.keys(formats).length === 0)
        return this.unwrap();
      const next2 = this.next;
      next2 instanceof _InlineBlot2 && next2.prev === this && isEqual2(formats, next2.formats()) && (next2.moveChildren(this), next2.remove());
    }
    replaceWith(name, value) {
  ...
  var Block = class extends BlockBlot$1 {
    ...
    split(index) {
      ...
      const next2 = super.split(index, force);
      this.cache = {};
      return next2;
    }
  };
  ...
  var Cursor = class _Cursor extends EmbedBlot$1 {
    ...
    restore() {
      ...
      if (range) {
        ...
        const start3 = remapOffset(range.start.node, range.start.offset);
        const end2 = remapOffset(range.end.node, range.end.offset);
  ...
  var instances_default2 = /* @__PURE__ */ new WeakMap();
  ...
  EVENTS.forEach((eventName) => {
    document.addEventListener(eventName, function() {
      ...
      Array.from(document.querySelectorAll(".ql-container")).forEach((node) => {
        const quill = instances_default2.get(node);
  ...
  var Selection = class {
    ...
    getBounds(index) {
      ...
      let node;
      let [leaf, offset2] = this.scroll.leaf(index);
      if (leaf == null)
        return null;
      if (length > 0 && offset2 === leaf.length()) {
        const [next2] = this.scroll.leaf(index + 1);
        if (next2) {
      ...
      if (length > 0) {
        ...
        [leaf, offset2] = this.scroll.leaf(index + length);
        if (leaf == null)
          return null;
        [node, offset2] = leaf.position(offset2, true);
    ...
    hasFocus() {
      return document.activeElement === this.root || document.activeElement != null && contains2(this.root, document.activeElement);
    }
    normalizedToRange(range) {
      ...
      const end2 = Math.min(Math.max(...indexes), this.scroll.length() - 1);
      const start3 = Math.min(end2, ...indexes);
      return new Range(start3, end2 - start3);
    }
  ...
  var Editor = class {
      ...
      normalizedOps.reduce((index, op) => {
        ...
        if (op.insert != null) {
          ...
          if (typeof op.insert === "string") {
            ...
            const [line, offset2] = this.scroll.line(index);
            ...
            if (isInlineEmbed) {
              ...
              const [leaf, offset2] = this.scroll.descendant(LeafBlot$1, index - 1);
    ...
    removeFormat(index, length) {
      ...
      const [line, offset2] = this.scroll.line(index + length);
      let suffixLength = 0;
  ...
  function convertHTML(blot, index, length) {
    ...
    if (blot instanceof ParentBlot$1) {
      if (blot.statics.blotName === "list-container") {
        const items = [];
        blot.children.forEachAt(index, length, (child, offset2, childLength) => {
      ...
      const [start3, end2] = outerHTML.split(`>${innerHTML}<`);
      if (start3 === "<table") {
        return `<table style="border: 1px solid #000;">${parts.join("")}<${end2}`;
      }
      return `${start3}>${parts.join("")}<${end2}`;
    }
  ...
  var Quill = class _Quill {
    ...
    static find(node) {
      ...
      return instances_default2.get(node) || globalRegistry.find(node, bubble);
    }
    static import(name) {
    ...
    constructor(container) {
      ...
      instances_default2.set(this.container, this);
  ...
  function shiftRange2(range, index, lengthOrSource, source) {
    ...
    if (index && typeof index.transformPosition === "function") {
      [start3, end2] = [range.index, range.index + range.length].map((pos) => (
  ...
  var Scroll = class extends ScrollBlot$1 {
    ...
    deleteAt(index, length) {
      const [first, offset2] = this.line(index);
...
${escapeText(this.code(index, length))}
  ...
  var Keyboard = class _Keyboard extends module_default {
    ...
    handleDelete(range, context) {
      ...
      if (context.offset >= line.length() - 1) {
        const [next2] = this.quill.getLine(range.index + 1);
        if (next2) {
          const curFormats = line.formats();
    ...
    bindings: {
      ...
      "table enter": {
        ...
        handler(range) {
          ...
          if (module2) {
            ...
            const shift = tableSide(table, row, cell, offset2);
            if (shift == null)
              return;
      ...
      "list autofill": {
        ...
        handler(range, context) {
          ...
          this.quill.history.cutoff();
          const delta = new import_quill_delta5.default().retain(range.index - offset2).delete(length + 1).retain(line.length() - 2 - offset2).retain(1, {
            list: value
          });
          this.quill.updateContents(delta, Quill.sources.USER);
          this.quill.history.cutoff();
      ...
      "code exit": {
        ...
        handler(range) {
          ...
          while (cur != null && cur.length() <= 1 && cur.formats()["code-block"]) {
            ...
            numLines -= 1;
            if (numLines <= 0) {
              const delta = new import_quill_delta5.default().retain(range.index + line.length() - offset2 - 2).retain(1, {
                "code-block": null
              }).delete(1);
              this.quill.updateContents(delta, Quill.sources.USER);
              this.quill.setSelection(range.index - 1, Quill.sources.SILENT);
  ...
  function matchNewline(node, delta, scroll) {
    if (!deltaEndsWith(delta, "\n")) {
      ...
      if (delta.length() > 0 && node.nextSibling) {
        let nextSibling2 = node.nextSibling;
        while (nextSibling2 != null) {
          if (isLine2(nextSibling2, scroll)) {
            return delta.insert("\n");
          }
          const match2 = scroll.query(nextSibling2);
          if (match2 && match2.prototype instanceof BlockEmbed) {
            return delta.insert("\n");
          }
          nextSibling2 = nextSibling2.firstChild;
        }
      }
    }
    return delta;
  ...
  function transformRange(range, delta) {
    if (!range)
      return range;
    const start3 = delta.transformPosition(range.index);
    const end2 = delta.transformPosition(range.index + range.length);
    return {
      index: start3,
      length: end2 - start3
    };
...
${escapeText(this.code(index, length))}
  ...
  var Tooltip2 = class {
    ...
    position(reference2) {
      const left2 = reference2.left + reference2.width / 2 - this.root.offsetWidth / 2;
      const top2 = reference2.bottom + this.quill.root.scrollTop;
      ...
      if (rootBounds.bottom > containerBounds.bottom) {
        ...
        const verticalShift = reference2.bottom - reference2.top + height;
  ...
  var BaseTooltip = class extends tooltip_default2 {
    constructor(quill, boundsContainer) {
